...
流程: 实体先抽 → LLM 对 entity pairs 判别式判断是否有关联 → 连边
图是纯粹的索引层，每条边直接指向来源句子 ID。

实体抽取: spaCy NER (快速) 或 LLM (精准)
关系连边: LLM 判别式 — 给定 sentence + entity pairs，二元判断 YES/NO + 关系类型
"""
...
class DiscriminativeRelationDetector:
    """判别式关系检测 — 先有实体，再判断实体对之间是否有关联

    流程: sentence + entity list → 生成 entity pairs → LLM 逐对判断 YES/NO
    图是纯粹的索引层，每条确认的边都携带 source_sentence_id。
    """
