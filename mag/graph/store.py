...
class GraphStore:
    ...
    def get_entity(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "linked_sentence_ids": node.get("linked_sentence_ids", []),
            "entity_type": node.get("entity_type", ""),
        }
    ...
    def get_neighbors_scoped(
        ...
        if max_hops == 1:
            neighbors = set()
            for _, neighbor, edge_data in g.out_edges(entity_name, data=True):
                if self._edge_matches_scope(edge_data, session_scope):
                    neighbors.add(neighbor)
            for predecessor, _, edge_data in g.in_edges(entity_name, data=True):
                if self._edge_matches_scope(edge_data, session_scope):
    ...
    def get_triples_for_sentence(self, sentence_id: str) -> List[Tuple[str, str, str]]:
        ...
        return self._sentence_triples.get(sentence_id, [])
    ...
    def get_sentences_for_entity(
        ...
        for node in g.nodes:
            if node.lower() == name_lower:
                direct = g.nodes[node].get("linked_sentence_ids", [])
                linked_scopes = g.nodes[node].get(NODE_SCOPES, {})
                for sid in direct:
    ...
    def get_sentences_for_relation(
        ...
        for key, edge_data in g[head].get(tail, {}).items():
    ...
    def get_all_relations_for_entity(
        ...
        # 出边
        for _, tail, edge_data in g.out_edges(entity_name, data=True):
            if not self._edge_matches_scope(edge_data, session_scope):
                continue
            relations.append({
                "head": entity_name,
                ...
                "source_sentence_ids": edge_data.get(REL_ORIGINS, []),
                "direction": "out",
            })

        # 入边
        for head, _, edge_data in g.in_edges(entity_name, data=True):
            if not self._edge_matches_scope(edge_data, session_scope):
                continue
            relations.append({
                "head": head,
